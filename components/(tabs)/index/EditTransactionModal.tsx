...
export const EditTransactionModal = ({ title, text, transaction, onConfirm, onCancel }: ConfirmModalProps) => {
    ...
    const { handleSetCategory, handleSetProduct, inputValue, inputState, onChangeInputText, onSubmitEditing, productSuggestions, categorySuggestions, quantity } = useTransactionInput(transaction);
    ...
    const quantitySelectLength = quantity.toString().length;
        ...
        <GestureHandlerRootView style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.5)' }} onTouchEnd={onCancel}>
            <View style={{ height: 300, justifyContent: 'center', alignItems: 'center', backgroundColor: COLORS.backgroundMiddle, padding: 20 }} onTouchEnd={e => e.stopPropagation()}>
                ...
                <Input
                    ...
                    selection={(inputState === InputStates.SELECTING_QUANTITY && select) ? { start: inputValue.length - quantitySelectLength, end: inputValue.length } : undefined}
                    value={inputValue}
